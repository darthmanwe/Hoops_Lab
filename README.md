...
![Tests](https://img.shields.io/badge/tests-408%20offline-199e70)
...
docs/social-preview.png  The GitHub/LinkedIn card, composed at 1280x640
scripts/           Screenshot capture and social-card rendering
```
...
npm run ml:test                  # 267 Python tests, offline, no credentials
...
Regenerating the images needs both dev servers up, since they are captured from
the running app rather than drawn:

```bash
npm run shots                    # the README screenshots, each asserted on content
npm run social-card              # the 1280x640 link-preview card
```
