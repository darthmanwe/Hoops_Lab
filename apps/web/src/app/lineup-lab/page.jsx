...
function LineupLabPageContent() {
  ...
  const initialPlayers = (
    searchParams.get("players") ?? "NBA_201939,NBA_2544,EL_9001,EL_9002,NBA_2544"
  ).split(",");
    ...
    <section className="space-y-6">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <div>
            <EntitySearchInput
              value={slot1}
              onChange={setSlot1}
              placeholder="Player 1 (name or ID)"
              type="player"
            />
            <p className="mt-1 text-xs text-slate-300">
          ...
          <div>
            <EntitySearchInput
              value={slot2}
              onChange={setSlot2}
              placeholder="Player 2 (name or ID)"
              type="player"
            />
            <p className="mt-1 text-xs text-slate-300">
          ...
          <div>
            <EntitySearchInput
              value={slot3}
              onChange={setSlot3}
              placeholder="Player 3 (name or ID)"
              type="player"
            />
            <p className="mt-1 text-xs text-slate-300">
          ...
          <div>
            <EntitySearchInput
              value={slot4}
              onChange={setSlot4}
              placeholder="Player 4 (name or ID)"
              type="player"
            />
            <p className="mt-1 text-xs text-slate-300">
          ...
          <div>
            <EntitySearchInput
              value={slot5}
              onChange={setSlot5}
              placeholder="Player 5 (name or ID)"
              type="player"
            />
            <p className="mt-1 text-xs text-slate-300">
      ...
      {!loading && !error && !data ? (
        <EmptyPanel
          ...
          description="Run a lineup calculation with five valid player IDs."
        />
      ) : null}
      ...
      {!loading && !error && data ? (
        <>
          <div className="grid gap-3 md:grid-cols-5">
            ...
            <MetricTile
              label="Off Projection"
              value={Number(metrics.offense_projection ?? 0).toFixed(2)}
            />
            <MetricTile
              label="Spacing Index"
              value={Number(metrics.spacing_index ?? 0).toFixed(2)}
            />
            <MetricTile
              label="Transition Fit"
              value={Number(metrics.transition_fit ?? 0).toFixed(2)}
            />
          ...
          {mode === "overview" || mode === "gravity" ? (
            <Card>
              <CardHeader
                title="Lineup Radar"
                subtitle="Multi-metric fit profile for the selected five."
              />
          ...
          {mode === "overview" || mode === "style" ? (
            <Card>
              <CardHeader
                title="Style Fit Bars"
                subtitle="Transition vs set-play derived impact."
              />
              <MetricBarChart
                data={styleBarData}
                xKey="metric"
