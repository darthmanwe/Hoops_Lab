...
export default function GameDetailPage({ params }) {
  ...
  if (!gameData.data?.game)
    return (
      <EmptyPanel title="Game not found" description="No detail available for this game id." />
    );
    ...
    <section className="space-y-6">
      <Card>
        ...
        <p className="mb-3 text-sm text-slate-300">
          Follow this matchup with three views: traditional production, fatigue pressure, and
          momentum swings.
        </p>
        <div className="grid gap-3 md:grid-cols-4">
          <MetricTile
            label="Home"
            value={game.home_team_name ?? game.home_team_id}
            hint={`Score ${game.home_score ?? "-"}`}
          />
          <MetricTile
            label="Away"
            value={game.away_team_name ?? game.away_team_id}
            hint={`Score ${game.away_score ?? "-"}`}
          />
      ...
      {mode === "box" ? (
        <Card>
          <CardHeader
            title="Boxscore View"
            subtitle="Points, assists, and rebounds by player line."
          />
          <MetricBarChart
            data={boxChartData}
            xKey="player"
          ...
          {fatigueData.data ? (
            <Card>
              ...
              <div className="grid gap-3 md:grid-cols-4">
                <MetricTile
                  label="Home Fatigue"
                  value={Number(fatigueData.data.home_fatigue_score ?? 0).toFixed(2)}
                />
                <MetricTile
                  label="Away Fatigue"
                  value={Number(fatigueData.data.away_fatigue_score ?? 0).toFixed(2)}
                />
                <MetricTile
                  label="Rest Disadvantage"
                  value={fatigueData.data.rest_disadvantage_flag ? "Yes" : "No"}
                />
                <MetricTile
                  label="Travel Disadvantage"
                  value={fatigueData.data.travel_disadvantage_flag ? "Yes" : "No"}
                />
              </div>
            </Card>
          ) : null}
      ...
      {mode === "momentum" ? (
        <>
          ...
          {momentumData.data ? (
            <Card>
              <CardHeader
                title="Momentum and Clutch"
                subtitle="Run strength and clutch net swings."
              />
              <div className="grid gap-3 md:grid-cols-4">
                <MetricTile
                  label="Best Run Team"
                  value={
                    momentumData.data.best_run_team_name ??
                    momentumData.data.best_run_team_abbrev ??
                    momentumData.data.best_run_team_id ??
                    "-"
                  }
                />
                <MetricTile
                  label="Best Run Points"
                  value={momentumData.data.best_run_points ?? 0}
                />
                <MetricTile
                  label="Swing Index"
                  value={Number(momentumData.data.swing_index ?? 0).toFixed(2)}
                />
                <MetricTile
                  label="Clutch Possessions"
                  value={momentumData.data.clutch_possessions ?? 0}
                />
              </div>
            </Card>
          ) : null}
