...
function GamesPageContent() {
  ...
  const [league, setLeague] = useState(
    searchParams.get("league")?.toUpperCase() === "EL" ? "EL" : "NBA"
  );
  const [season, setSeason] = useState(
    searchParams.get("season") ??
      DEFAULT_SEASON_BY_LEAGUE[searchParams.get("league")?.toUpperCase() === "EL" ? "EL" : "NBA"]
  );
  ...
  return (
    <section className="space-y-6">
      <Card>
        <CardHeader
          title="Games Center"
          subtitle="Find a game and open its full story: box score leaders, momentum swings, and fatigue context."
        />
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <SearchInput
            value={query}
            onChange={setQuery}
            placeholder="Search by game, team name, or team ID..."
            className="md:w-80"
          />
        </div>
      </Card>
      ...
      {!loading && !error && filtered.length === 0 ? (
        <EmptyPanel
          ...
          description="Try another league/season or adjust your filter."
        />
      ) : null}
      ...
      {!loading && !error && filtered.length > 0 ? (
        <Card>
          <CardHeader
            title={`${filtered.length} Games`}
            subtitle={`Season: ${seasonLabel(season)}`}
          />
            ...
            {filtered.map((game) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-slate-300">
                  {formatGameDate(game.game_date)} · {game.game_id}
                </p>
                <p className="mt-2 text-sm text-neon-300">
                  {game.home_team_abbrev ?? game.home_team_id}{" "}
                  <span className="text-slate-400">vs</span>{" "}
                  {game.away_team_abbrev ?? game.away_team_id}
                </p>
                <p className="mt-1 text-xs text-slate-300">
