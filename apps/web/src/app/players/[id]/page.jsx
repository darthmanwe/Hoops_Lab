...
export default function PlayerPage({ params, searchParams }) {
  ...
  const playerData = useApi(
    `/players/${encodeURIComponent(playerId)}?season=${encodeURIComponent(season)}`
  );
  const compsData = useApi(
    `/players/${encodeURIComponent(playerId)}/comps?season=${encodeURIComponent(season)}&k=10`
  );
  const shotData = useApi(
    `/players/${encodeURIComponent(playerId)}/shot-profile?season=${encodeURIComponent(season)}`
  );
  const translationData = useApi(
    `/players/${encodeURIComponent(playerId)}/translation?season=${encodeURIComponent(season)}`
  );
  ...
  if (!playerData.data?.player)
    return <EmptyPanel title="Player not found" description="Try another player id." />;
    ...
    <section className="space-y-6">
      <Card>
        ...
        <p className="mb-3 text-sm text-slate-300">
          Use this page to understand player style: who they resemble, where they shoot from, and
          how well production translates across leagues.
        </p>
        <div className="mb-3 grid gap-2 md:grid-cols-3">
          ...
          <Button
            ...
            onClick={() => {
              ...
              router.replace(
                `/players/${encodeURIComponent(playerInput.trim() || playerId)}?season=${encodeURIComponent(nextSeason)}`
              );
            }}
          >
            Open Player
        ...
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <MetricTile
            label="Clutch Impact"
            value={Number(features.clutch_impact ?? 0).toFixed(2)}
          />
      ...
      {mode === "comps" ? (
        <Card>
          <CardHeader
            title="Closest Archetype Comps"
            subtitle="Most similar player styles for the selected season."
          />
            ...
            <div className="space-y-2">
              {comps.map((comp, index) => (
                ...
                >
                  ...
                  <p className="text-sm font-semibold text-neon-300">
                    {Number(comp.score).toFixed(3)}
                  </p>
                </div>
              ))}
            </div>
      ...
      {mode === "translation" ? (
        <Card>
          <CardHeader
            ...
            subtitle="Cross-league standardized equivalence panel."
          />
          ...
          {!translationData.loading && !translationData.error ? (
            <div className="grid gap-3 md:grid-cols-4">
              <MetricTile
                label="Std Usage"
                value={Number(translation.standardized_usage ?? 0).toFixed(2)}
              />
              <MetricTile
                label="Std TS"
                value={Number(translation.standardized_ts ?? 0).toFixed(2)}
              />
              <MetricTile
                label="Translation Score"
                value={Number(translation.translation_score ?? 0).toFixed(2)}
              />
              <MetricTile
                label="NBA Eq Rating"
                value={Number(translation.nba_equivalent_rating ?? 0).toFixed(1)}
              />
            </div>
          ) : null}
        </Card>
